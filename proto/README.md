...
```
proto/pharos/buoy/v1/control.proto   node control service (coxswain ↔ node)
                                     (path keeps the legacy "buoy" dir until the proto tree is renamed)
```
...
- `coxswain` owns these contracts; `node`, `relay`, and `caravel` consume them.
